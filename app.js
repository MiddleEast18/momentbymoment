(() => {
  ...
  const CONFIG = {
    ...
    NEWS_CACHE_URL: 'https://dndlkenyfymlrjnslyzb.supabase.co/functions/v1/news-cache',
    TABLE: 'news_articles',
  ...
  // List view fields only. Heavy detail fields (raw_payload, agency_urls, revisions) are loaded on demand.
  ...
  function effectivePublishedAt(article) { const published = new Date(article?.published_at || 0).getTime(); const now = Date.now(); if (Number.isFinite(published) && published <= now + 5 * 60 * 1000) return article?.published_at; return article?.created_at || article?.updated_at || article?.published_at; }
  function formatArticleTime(article) { return formatRelativeTime(effectivePublishedAt(article)); }
  ...
  function populateCard(node, article) { node.className = ['card', layoutClass(article.layout_size)].filter(Boolean).join(' '); node.dataset.id = article.id; node.dataset.sentiment = article.sentiment || 'Neutral'; node.dataset.tier = String(importanceTier(article.importance_score)); node.querySelector('.card__category').textContent = MirsadText.normalize(CATEGORY_LABELS[article.category] || article.category || 'عام'); node.querySelector('.card__dot').style.background = CATEGORY_COLORS[article.category] || '#888'; node.querySelector('.card__source').textContent = MirsadText.normalize(article.source_name || article.source_domain || 'مصدر'); node.querySelector('.card__headline').textContent = MirsadText.normalize(article.headline || article.title || ''); node.querySelector('.card__summary').textContent = MirsadText.normalize(article.summary || ''); const timeEl = node.querySelector('.card__time'); timeEl.textContent = formatArticleTime(article); timeEl.setAttribute('datetime', effectivePublishedAt(article) || ''); node.querySelector('.card__confidence').textContent = `ثقة ${Math.round(Number(article.confidence_score ?? 0))}%`; const evidenceEl = node.querySelector('.card__evidence'); const evidence = []; if (article.claim_digest) evidence.push(MirsadText.normalize(typeof article.claim_digest === 'string' ? article.claim_digest : article.claim_digest.main_claim || '')); if (article.evidence_summary) evidence.push(MirsadText.normalize(article.evidence_summary)); if (!evidence.length && Number(article.source_count || 0) > 1) evidence.push(`مجمّع من ${article.source_count} مصادر`); evidenceEl.textContent = evidence.filter(Boolean).join(' · '); const updateBadge = node.querySelector('.card__update-badge'); updateBadge.hidden = Number(article.update_count || 0) <= 0; if (!updateBadge.hidden) updateBadge.textContent = `+${article.update_count} تحديث`; const verificationBadge = node.querySelector('.card__verification-badge'); verificationBadge.hidden = !article.is_pending_verification; if (article.is_pending_verification) verificationBadge.textContent = 'بانتظار التحقق'; }
  ...
  function normalizeForSearch(value) { return MirsadText.search(value); }
  ...
  function getTime(article, key) { const sourceKey = key === 'published_at' ? effectivePublishedAt(article) : article?.[key]; const value = new Date(sourceKey || 0).getTime(); return Number.isFinite(value) ? value : 0; }
  ...
  function getUrgentRows() { const candidates = state.articles.filter((a) => !a.is_pending_verification && isArabicHeadline(MirsadText.normalize(a.headline))); const rows = []; const seen = new Set(); for (const article of [...candidates].sort(compareLatest)) { const key = normalizeForSearch(article.headline); if (!key || seen.has(key)) continue; seen.add(key); rows.push(article); if (rows.length >= 4) break; } return rows; }
  function renderTicker() { const rows = getUrgentRows(); tickerTrackEl.replaceChildren(); const items = rows.length ? rows : [{ headline: 'لا توجد أخبار عاجلة جديدة — آخر المستجدات معروضة أدناه' }]; for (const item of items) { const span = document.createElement('span'); span.className = 'ticker-strip__item'; span.textContent = MirsadText.normalize(item.headline || item.title || 'خبر جديد'); tickerTrackEl.appendChild(span); } }
  ...
  async function fetchInitialBatch() { const { data, error } = await sb.from(CONFIG.TABLE).select(SELECT_COLUMNS).eq('is_pending_verification', false).order('published_at', { ascending: false, nullsFirst: false }).order('updated_at', { ascending: false, nullsFirst: false }).order('created_at', { ascending: false, nullsFirst: false }).limit(CONFIG.INITIAL_FETCH_LIMIT); if (error) { console.error('[mirsad] initial fetch failed', error); return []; } return (data || []).map((row) => ({ ...row, headline: MirsadText.normalize(row.headline || row.title || ''), summary: MirsadText.normalize(row.summary || '') })).filter((row) => isArabicHeadline(row.headline)).sort(compareLatest); }
  async function fetchLatestViaRest() { const params = new URLSearchParams({ select: SELECT_COLUMNS, is_pending_verification: 'eq.false', order: 'published_at.desc.nullslast,updated_at.desc.nullslast,created_at.desc.nullslast', limit: String(CONFIG.INITIAL_FETCH_LIMIT) }); try { const response = await fetch(`${CONFIG.SUPABASE_URL}/rest/v1/${CONFIG.TABLE}?${params.toString()}`, { headers: { apikey: CONFIG.SUPABASE_ANON_KEY, Authorization: `Bearer ${CONFIG.SUPABASE_ANON_KEY}` } }); if (!response.ok) throw new Error(`HTTP ${response.status}`); return (await response.json()).map((row) => ({ ...row, headline: MirsadText.normalize(row.headline || row.title || ''), summary: MirsadText.normalize(row.summary || '') })).filter((row) => isArabicHeadline(row.headline)).sort(compareLatest); } catch (error) { console.error('[mirsad] REST fetch failed', error); return []; } }
  async function fetchLatestViaCache() { try { const response = await fetch(CONFIG.NEWS_CACHE_URL, { headers: { apikey: CONFIG.SUPABASE_ANON_KEY, Authorization: `Bearer ${CONFIG.SUPABASE_ANON_KEY}`, accept: 'application/json' } }); if (!response.ok) throw new Error(`HTTP ${response.status}`); return (await response.json()).map((row) => ({ ...row, headline: MirsadText.normalize(row.headline || row.title || ''), summary: MirsadText.normalize(row.summary || '') })).filter((row) => isArabicHeadline(row.headline)).sort(compareLatest); } catch (error) { console.warn('[mirsad] shared cache unavailable, using direct REST', error); return fetchLatestViaRest(); } }
  async function initialLoad() { loadPreferences(); syncControlsFromState(); setConnectionState('connecting'); const rows = await fetchLatestViaCache(); state.articles = rows.slice(0, CONFIG.MAX_KEPT_ARTICLES).sort(compareLatest); renderWithFlip(); state.featuredLoading = false; renderFeaturedRail(getVisibleArticles()); touchLastSync(); setConnectionState('live'); startRealtime(); startPollingFallback(); }
  function startRealtime() { if (state.channel) sb.removeChannel(state.channel); state.channel = sb.channel('mirsad-news-live').on('postgres_changes', { event: 'INSERT', schema: 'public', table: CONFIG.TABLE }, ({ new: row }) => { if (row?.is_pending_verification || !isArabicHeadline(row?.headline || row?.title)) return; upsertArticle(row); renderWithFlip(); touchLastSync(); }).on('postgres_changes', { event: 'UPDATE', schema: 'public', table: CONFIG.TABLE }, ({ new: row }) => { if (row?.is_pending_verification || !isArabicHeadline(row?.headline || row?.title)) return; upsertArticle(row); renderWithFlip(row?.id); touchLastSync(); }).subscribe((status) => { if (status === 'SUBSCRIBED') { setConnectionState('live'); } if (status === 'CHANNEL_ERROR' || status === 'TIMED_OUT' || status === 'CLOSED') { setConnectionState('polling'); startPollingFallback(); } }); }
  async function refreshFromServer() { const rows = await fetchLatestViaCache(); if (rows.length) { state.articles = rows.slice(0, CONFIG.MAX_KEPT_ARTICLES).sort(compareLatest); renderWithFlip(); touchLastSync(); } }
  function startPollingFallback() { if (state.pollTimer) return; state.pollTimer = setInterval(refreshFromServer, CONFIG.POLL_FALLBACK_INTERVAL_MS); }
  function stopPollingFallback() { if (!state.pollTimer) return; clearInterval(state.pollTimer); state.pollTimer = null; }
