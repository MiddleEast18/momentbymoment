(() => {
  ...
  function renderFeaturedRail(visibleArticles) { const loader = document.getElementById('featuredLoader'); featuredRailEl.innerHTML = ''; if (loader) featuredRailEl.appendChild(loader); const largeCards = visibleArticles.filter((a) => a.layout_size === 'large').slice(0, 5); const list = largeCards.length ? largeCards : visibleArticles.slice(0, 4); if (loader) loader.hidden = !state.featuredLoading; list.forEach((article) => { const card = createCardElement(article); card.classList.add(largeCards.length ? 'card-large' : 'card-medium'); card.style.minWidth = '86%'; card.style.scrollSnapAlign = 'center'; featuredRailEl.appendChild(card); }); }
