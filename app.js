(() => {
  ...
  function formatArticleTime(article) { return formatRelativeTime(article?.published_at); }
  ...
  function populateCard(node, article) { node.className = ['card', layoutClass(article.layout_size)].filter(Boolean).join(' '); node.dataset.id = article.id; node.dataset.sentiment = article.sentiment || 'Neutral'; node.dataset.tier = String(importanceTier(article.importance_score)); node.querySelector('.card__score').textContent = String(article.importance_score ?? ''); node.querySelector('.card__category').textContent = MirsadText.normalize(CATEGORY_LABELS[article.category] || article.category || 'عام'); node.querySelector('.card__dot').style.background = CATEGORY_COLORS[article.category] || '#888'; node.querySelector('.card__source').textContent = MirsadText.normalize(article.source_name || article.source_domain || 'مصدر'); node.querySelector('.card__headline').textContent = MirsadText.normalize(article.headline || article.title || ''); node.querySelector('.card__summary').textContent = MirsadText.normalize(article.summary || ''); const timeEl = node.querySelector('.card__time'); timeEl.textContent = formatArticleTime(article); timeEl.setAttribute('datetime', article.published_at || ''); node.querySelector('.card__confidence').textContent = `ثقة ${Math.round(Number(article.confidence_score ?? 0))}%`; const evidenceEl = node.querySelector('.card__evidence'); const evidence = []; if (article.claim_digest) evidence.push(MirsadText.normalize(typeof article.claim_digest === 'string' ? article.claim_digest : article.claim_digest.main_claim || '')); if (article.evidence_summary) evidence.push(MirsadText.normalize(article.evidence_summary)); if (!evidence.length && Number(article.source_count || 0) > 1) evidence.push(`مجمّع من ${article.source_count} مصادر`); evidenceEl.textContent = evidence.filter(Boolean).join(' · '); const updateBadge = node.querySelector('.card__update-badge'); updateBadge.hidden = Number(article.update_count || 0) <= 0; if (!updateBadge.hidden) updateBadge.textContent = `+${article.update_count} تحديث`; const verificationBadge = node.querySelector('.card__verification-badge'); verificationBadge.hidden = !article.is_pending_verification; if (article.is_pending_verification) verificationBadge.textContent = 'بانتظار التحقق'; }
