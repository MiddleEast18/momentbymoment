(() => {
  ...
  function formatArticleTime(article) { if (article?.source_name === 'Al Riyadh') { const time = new Date(article.published_at || 0).getTime(); return Number.isFinite(time) && time > 0 ? `تاريخ الإصدار: ${new Intl.DateTimeFormat('ar', { dateStyle: 'medium' }).format(new Date(time))}` : 'تاريخ الإصدار غير محدد'; } return formatRelativeTime(article?.published_at); }
