(() => {
  ...
  function decodeEntities(value) {
    ...
    if (!input || !/&(?:[a-zA-Z][a-zA-Z0-9]+|#\d+|#x[\da-fA-F]+);/.test(input)) return input;
  ...
  function getUrgentRows() { const candidates = state.articles.filter((a) => !a.is_pending_verification && isArabicHeadline(a.headline)); const rows = []; const seen = new Set(); for (const article of [...candidates].sort(compareLatest)) { const key = normalizeForSearch(decodeEntities(article.headline)).replace(/\s+/g, ' ').trim(); if (!key || seen.has(key)) continue; seen.add(key); rows.push(article); if (rows.length >= 4) break; } return rows; }
  function renderTicker() { const rows = getUrgentRows(); tickerTrackEl.innerHTML = ''; const items = rows.length ? rows : [{ headline: 'لا توجد أخبار عاجلة جديدة — آخر المستجدات معروضة أدناه' }]; for (const item of items) { const span = document.createElement('span'); span.className = 'ticker-strip__item'; span.textContent = decodeEntities(item.headline || item.title || 'خبر جديد'); tickerTrackEl.appendChild(span); } }
