(() => {
  ...
  function loadPreferences() { try { const raw = localStorage.getItem(STORAGE_KEY); if (!raw) return; const parsed = JSON.parse(raw); if (parsed && typeof parsed === 'object') { state.activeFilter = parsed.activeFilter || state.activeFilter; state.searchQuery = parsed.searchQuery || state.searchQuery; state.sortMode = parsed.sortMode || state.sortMode; } } catch {} }
  ...
  function getVisibleArticles() { const q = normalizeForSearch(state.searchQuery); return state.articles.filter((a) => !a.is_pending_verification).filter((a) => state.activeFilter === 'all' || a.category === state.activeFilter).filter((a) => !q || normalizeForSearch(`${a.headline || a.title || ''} ${a.summary || ''} ${a.source_name || ''}`).includes(q)); }

  function getTime(article, key) {
    const value = new Date(article?.[key] || 0).getTime();
    return Number.isFinite(value) ? value : 0;
  }
  function compareLatest(a, b) {
    return getTime(b, 'published_at') - getTime(a, 'published_at') || getTime(b, 'updated_at') - getTime(a, 'updated_at') || getTime(b, 'created_at') - getTime(a, 'created_at') || String(b?.id || '').localeCompare(String(a?.id || ''));
  }
  function comparePriority(a, b) {
    return (Number(b?.importance_score) || 0) - (Number(a?.importance_score) || 0) || (Number(b?.update_count) || 0) - (Number(a?.update_count) || 0) || getTime(b, 'updated_at') - getTime(a, 'updated_at') || getTime(b, 'published_at') - getTime(a, 'published_at') || getTime(b, 'created_at') - getTime(a, 'created_at') || String(b?.id || '').localeCompare(String(a?.id || ''));
  }
  function compareUpdates(a, b) {
    return (Number(b?.update_count) || 0) - (Number(a?.update_count) || 0) || getTime(b, 'updated_at') - getTime(a, 'updated_at') || getTime(b, 'published_at') - getTime(a, 'published_at') || getTime(b, 'created_at') - getTime(a, 'created_at') || String(b?.id || '').localeCompare(String(a?.id || ''));
  }
  function sortArticles(items) {
    const copy = [...items];
    if (state.sortMode === 'latest') return copy.sort(compareLatest);
    if (state.sortMode === 'updates') return copy.sort(compareUpdates);
  ...
  function getUrgentRows() {
    ...
    const rows = [];
    const seen = new Set();
    for (const article of [...candidates].sort(compareLatest)) {
