(() => {
  ...
  const load = async () => { const { data, error } = await sb.from('rapid_news').select('id,source_key,source_name,source_url,headline,summary,published_at,received_at,importance_score').eq('source_key', 'daraj').order('published_at', { ascending: false, nullsFirst: false }).order('received_at', { ascending: false, nullsFirst: false }).limit(400); if (error) { status.textContent = 'تعذر تحميل الأخبار الملتقطة'; return; } render(data || []); status.textContent = currentRows.length ? `${currentRows.length} خبرًا ظاهرًا` : 'بانتظار الأخبار الملتقطة'; };
