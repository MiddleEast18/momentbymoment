(() => {
  ...
  const consume = async (kind) => { if (!window.mirsadViewAccess || !ACCESS_KINDS.has(kind)) return false; more.disabled = true; all.disabled = true; const result = await window.mirsadViewAccess.consume(kind); more.disabled = false; all.disabled = false; if (!result || result.allowed !== true) { const message = result?.error?.message; status.textContent = message === 'not_authenticated' ? 'سجّل الدخول' : (result?.error ? 'تعذر التحقق، حاول مجددًا' : 'لا توجد فتحات كافية'); return false; } status.textContent = result.charged ? `خُصمت ${kind === 'more' ? 5 : 100} فتحات` : 'مجاني هذه الدورة'; return true; };
