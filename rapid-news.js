(() => {
  ...
  const showAccessNotice = (text) => { let notice = document.getElementById('rapidNewsNotice'); if (!notice) { notice = document.createElement('div'); notice.id = 'rapidNewsNotice'; notice.className = 'rapid-news__notice'; document.body.appendChild(notice); } notice.textContent = text; notice.classList.add('is-visible'); clearTimeout(noticeTimer); noticeTimer = setTimeout(() => notice.classList.remove('is-visible'), 3200); };
  ...
  const load = async () => { const { data, error } = await sb.from('rapid_news').select('id,source_key,source_name,source_url,headline,summary,published_at,received_at,importance_score').eq('source_key', 'middleeasteye').order('published_at', { ascending: false, nullsFirst: false }).order('received_at', { ascending: false, nullsFirst: false }).limit(400); if (error) { status.textContent = 'تعذر تحميل الأخبار الملتقطة'; return; } render(data || []); status.textContent = currentRows.length ? `${currentRows.length} خبرًا ظاهرًا` : 'بانتظار الأخبار الملتقطة'; };
  ...
  const open = async () => { if (!window.mirsadViewAccess) return; const access = await window.mirsadViewAccess.openRapid(); if (!access.allowed) { status.textContent = access.error?.message === 'not_authenticated' ? 'سجّل الدخول' : 'تعذر الفتح، حاول مجددًا'; await refreshUnread(); return; } opened = true; rail.hidden = false; toggle.setAttribute('aria-expanded', 'true'); status.textContent = access.unread_count ? `تم عرض ${access.unread_count} خبرًا جديدًا` : 'لا أخبار جديدة غير مقروءة'; await refreshUnread(); };
  const consume = async (kind) => { if (!window.mirsadViewAccess || !ACCESS_KINDS.has(kind)) return false; more.disabled = true; all.disabled = true; const result = await window.mirsadViewAccess.consume(kind); more.disabled = false; all.disabled = false; if (!result || result.allowed !== true) { const message = result?.error?.message; const notice = message === 'not_authenticated' ? 'سجّل الدخول' : (result?.error ? 'تعذر التحقق، حاول مجددًا' : 'لا توجد فتحات كافية'); status.textContent = notice; showAccessNotice(notice); return false; } status.textContent = result.charged ? `خُصمت ${kind === 'more' ? 5 : 100} فتحات` : 'مجاني هذه الدورة'; return true; };
