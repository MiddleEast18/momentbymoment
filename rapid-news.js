(() => {
  ...
  const load = async () => { const { data, error } = await sb.from('rapid_news').select('id,source_key,source_name,source_url,headline,summary,published_at,received_at,importance_score').eq('source_key', 'middleeasteye').order('published_at', { ascending: false, nullsFirst: false }).order('received_at', { ascending: false, nullsFirst: false }).limit(400); if (error) { status.textContent = 'تعذر تحميل أخبار الشرق الأوسط بالإنجليزية'; return; } render(data || []); status.textContent = currentRows.length ? `${currentRows.length} خبرًا ظاهرًا` : 'بانتظار أخبار الشرق الأوسط بالإنجليزية'; };
  ...
  const open = async () => { if (!window.mirsadViewAccess) return; const access = await window.mirsadViewAccess.openRapid(); if (!access.allowed) { status.textContent = access.error?.message === 'not_authenticated' ? 'سجّل الدخول' : 'تعذر الفتح، حاول مجددًا'; await refreshUnread(); return; } opened = true; rail.hidden = false; toggle.setAttribute('aria-expanded', 'true'); status.textContent = access.unread_count ? `تم عرض ${access.unread_count} خبرًا جديدًا` : 'لا أخبار جديدة غير مقروءة'; const { data: { session: latestSession } = {} } = await sb.auth.getSession(); currentUser = Boolean(latestSession?.user); if (currentUser) { const { error: seenError } = await sb.rpc('rapid_news_mark_seen'); if (!seenError) { setBadge(0); return; } } await refreshUnread(); };
  const consume = async (kind) => { if (!window.mirsadViewAccess || !ACCESS_KINDS.has(kind)) return false; more.disabled = true; all.disabled = true; let result; try { result = await window.mirsadViewAccess.consume(kind); } catch (error) { result = { allowed: false, error }; } more.disabled = false; all.disabled = false; if (!result || result.allowed !== true) { const message = String(result?.error?.message || result?.error?.code || '').toLowerCase(); const requiresLogin = !currentUser || message.includes('not_authenticated') || message.includes('jwt') || message.includes('unauthorized') || message === '401'; const notice = requiresLogin ? 'سجّل الدخول' : (result?.error ? 'تعذر التحقق، حاول مجددًا' : 'لا توجد فتحات كافية'); status.textContent = notice; showAccessNotice(notice); return false; } status.textContent = result.charged ? `خُصمت ${kind === 'more' ? 5 : 100} فتحات` : 'مجاني هذه الدورة'; return true; };
