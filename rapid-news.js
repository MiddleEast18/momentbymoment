(() => {
  ...
  const open = async () => { if (!window.mirsadViewAccess) return; const access = await window.mirsadViewAccess.openRapid(); if (!access.allowed) { status.textContent = access.error?.message === 'not_authenticated' ? 'سجّل الدخول لعرض الأخبار الملتقطة' : `لا توجد فتحات كافية لعرض ${access.unread_count || 0} أخبار`; await refreshUnread(); return; } opened = true; rail.hidden = false; toggle.setAttribute('aria-expanded', 'true'); status.textContent = access.charged ? `تم خصم ${access.unread_count} فتحات لعرض الأخبار الجديدة` : (access.unread_count ? `تم عرض ${access.unread_count} خبرًا جديدًا` : 'لا توجد أخبار جديدة غير مقروءة'); await refreshUnread(); };
