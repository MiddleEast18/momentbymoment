(() => {
  ...
  const consume = async (kind) => { if (!window.mirsadViewAccess || !ACCESS_KINDS.has(kind)) return false; more.disabled = true; all.disabled = true; const result = await window.mirsadViewAccess.consume(kind); more.disabled = false; all.disabled = false; if (!result || result.allowed !== true) { const message = result?.error?.message; status.textContent = message === 'not_authenticated' ? 'سجّل الدخول لاستخدام الفتحات' : (result?.error ? 'تعذر التحقق من الفتحات حاليًا. حاول مرة أخرى.' : 'لا توجد فتحات كافية لهذه العملية.'); return false; } status.textContent = result.charged ? `تم خصم ${kind === 'more' ? 5 : 100} فتحات` : 'الاستخدام مجاني في هذه الدورة'; return true; };
