(() => {
  ...
  const open = async () => { if (!window.mirsadViewAccess) return; const access = await window.mirsadViewAccess.openRapid(); if (!access.allowed) { showRapidNotice(access.error?.message === 'not_authenticated' ? 'سجّل الدخول' : 'تعذر الفتح، حاول مجددًا'); status.textContent = ''; await refreshUnread(); return; } opened = true; rail.hidden = false; toggle.setAttribute('aria-expanded', 'true'); showRapidNotice(access.unread_count ? `تم عرض ${access.unread_count} خبرًا جديدًا` : 'لا أخبار جديدة غير مقروءة'); status.textContent = ''; const { data: { session: latestSession } = {} } = await sb.auth.getSession(); currentUser = Boolean(latestSession?.user); if (currentUser) { const { error: seenError } = await sb.rpc('rapid_news_mark_seen'); if (!seenError) { setBadge(0); return; } } await refreshUnread(); };
  ...
  list.addEventListener('click', (event) => { const link = event.target.closest?.('a[href]'); if (!link) return; event.preventDefault(); if (typeof window.mirsadOpenOriginalSource === 'function') void window.mirsadOpenOriginalSource(link.href); });
