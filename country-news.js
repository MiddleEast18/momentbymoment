(() => {
  ...
  const translationButton = (article) => foreignLanguage(article.language, article.country_news_sources?.source_key, article.headline, article.summary)
    ? `<button class="country-translate" type="button" data-article-id="${esc(article.id)}" aria-label="ترجمة عنوان وملخص الخبر إلى العربية">ترجمة إلى العربية</button>` : '';
  ...
  const translateArticle = async (button) => {
    ...
    try {
      const result = await fetch(TRANSLATE_FUNCTION, { method: 'POST', headers: { 'Content-Type': 'application/json', apikey: SUPABASE_KEY, Authorization: `Bearer ${SUPABASE_KEY}` }, body: JSON.stringify({ article_id: articleId, target_language: 'ar' }) }).then(async (response) => { const data = await response.json(); if (!response.ok) throw new Error(data.error || 'Translation failed'); return data; });
    ...
    } catch (error) { console.error('[mirsad article translation]', error); button.disabled = false; button.textContent = 'إعادة المحاولة'; }
  };
