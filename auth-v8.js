(() => {
  ...
  function injectStyles(){
    ...
    const s=document.createElement('style');s.id='mirsadAuthStyles';s.textContent=`
      ...
      .mirsad-profile-banner{position:fixed;left:16px;right:16px;bottom:16px;z-index:4500;display:flex;align-items:center;justify-content:space-between;gap:14px;padding:14px 16px;border:1px solid var(--panel-border-strong);border-radius:14px;background:rgba(16,21,28,.97);box-shadow:0 12px 40px rgba(0,0,0,.28)}\n      .mirsad-logout-animation{position:fixed;inset:0;z-index:99999;display:grid;place-items:center;background:rgba(8,11,15,.3);overflow:hidden;pointer-events:auto;transition:background .18s ease}.mirsad-logout-animation svg{display:block;width:100%;height:120px;overflow:visible}.mirsad-logout-animation path{fill:none;stroke-linecap:round;stroke-dasharray:1;stroke-dashoffset:1;animation:mirsad-logout-thread 1.15s cubic-bezier(.22,.72,.25,1) forwards}.mirsad-logout-animation .mirsad-logout-thread-shadow{stroke:rgba(201,162,39,.22);stroke-width:7;filter:blur(5px)}.mirsad-logout-animation .mirsad-logout-thread-main{stroke:var(--gold,#c9a227);stroke-width:1.8;filter:drop-shadow(0 0 6px rgba(201,162,39,.9))}.mirsad-logout-animation circle{fill:#f5d76e;filter:drop-shadow(0 0 7px rgba(201,162,39,.95))}.mirsad-logout-animation.is-complete{background:rgba(8,11,15,.08)}@keyframes mirsad-logout-thread{0%{stroke-dashoffset:1;opacity:0}10%{opacity:1}100%{stroke-dashoffset:0;opacity:1}}@media(prefers-reduced-motion:reduce){.mirsad-logout-animation path{animation-duration:.08s}.mirsad-logout-animation circle{display:none}}
  ...
  const PROFILE_LATER_KEY='mirsad.profile-later.v1';
  ...
  function playGoldBurst(origin, done){
    ...
    const layer=document.createElement('div');layer.className='mirsad-gold-burst';layer.setAttribute('aria-hidden','true');
    const rect=origin.getBoundingClientRect();
    const cx=rect.left+rect.width/2, cy=rect.top+rect.height/2;
    const bursts=[{x:cx,y:cy,n:16},{x:cx-28,y:cy-56,n:10},{x:cx+24,y:cy-38,n:8}];
    bursts.forEach((b,i)=>{
      const flash=document.createElement('span');flash.className='mirsad-gold-burst__flash';flash.style.left=b.x+'px';flash.style.top=b.y+'px';flash.style.animationDelay=(i*70)+'ms';layer.appendChild(flash);
      for(let k=0;k<b.n;k++){
        const spark=document.createElement('span');
        spark.className='mirsad-gold-burst__spark'+(k%4===0?' is-streak':'');
        const ang=(Math.PI*2*k)/b.n+(i*0.35);
        ...
        spark.style.left=b.x+'px';spark.style.top=b.y+'px';
        spark.style.animationDelay=(i*80+Math.random()*90)+'ms';
        layer.appendChild(spark);
      }
    });
    document.body.appendChild(layer);
    setTimeout(()=>{layer.remove();finish()},980);
  }
  ...
  function renderProfilePage(user,profile){
    ...
    root.querySelector('[data-delete]').addEventListener('click',async()=>{
      ...
      if(!await showDeleteAccountConfirm(await ensureUnlockAccount()))return;
  ...
  function showProfileBanner(user){
    if(profileLater(user)||document.getElementById('mirsadProfileBanner'))return;
    const b=document.createElement('aside');b.id='mirsadProfileBanner';b.className='mirsad-profile-banner';b.innerHTML=`<div class="mirsad-profile-banner__text"><strong>أكمل ملفك الشخصي</strong><span>أضف معلوماتك لتخصيص تجربتك في مِرصاد.</span></div><div class="mirsad-profile-banner__actions"><button class="mirsad-auth-button primary" type="button" data-open>إكمال الملف</button><button class="mirsad-auth-button" type="button" data-later>لاحقًا</button></div>`;document.body.appendChild(b);b.querySelector('[data-open]').addEventListener('click',()=>{clearProfileLater();openProfile(user)});b.querySelector('[data-later]').addEventListener('click',()=>{setProfileLater(user);b.remove()});
  }

  let logoutAnimationInFlight=false;
  function playLogoutAnimation(){
    if(logoutAnimationInFlight)return Promise.resolve(null);
    logoutAnimationInFlight=true;
    const layer=document.createElement('div');
    layer.className='mirsad-logout-animation';
    layer.setAttribute('role','status');
    layer.setAttribute('aria-label','جارٍ تسجيل الخروج');
    const path='M-30 63 C 32 45 52 81 108 61 S 183 35 242 63 S 318 91 378 58 S 452 28 512 61 S 588 87 650 59 S 725 39 786 64 S 862 84 923 57 S 1002 31 1060 62 S 1135 84 1230 52';
    layer.innerHTML=`<svg viewBox="0 0 1200 120" preserveAspectRatio="none" aria-hidden="true"><path class="mirsad-logout-thread-shadow" d="${path}" pathLength="1"></path><path class="mirsad-logout-thread-main" d="${path}" pathLength="1"></path><circle r="3"><animateMotion dur="1.15s" fill="freeze" path="${path}"></animateMotion></circle></svg>`;
    document.body.appendChild(layer);
    return new Promise(resolve=>{window.setTimeout(()=>resolve(layer),1150)});
  }
  function completeLogoutAnimation(layer){
    if(!layer)return;
