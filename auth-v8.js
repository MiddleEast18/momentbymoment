(() => {
  ...
  function renderProfilePage(user,profile){
    ...
    root.querySelector('[data-save]').addEventListener('click',async()=>{const status=root.querySelector('#mirsadProfilePageStatus'),btn=root.querySelector('[data-save]');const payload={id:user.id,display_name:root.querySelector('#profilePageName').value.trim(),avatar_url:avatarUrl(root.querySelector('#profilePageAvatar').value)||null,username:root.querySelector('#profilePageUsername').value.trim()||null,bio:root.querySelector('#profilePageBio').value.trim()||null,onboarding_completed:true};btn.disabled=true;statusText(status,'جارٍ الحفظ…');const{error}=await sb.from(CONFIG.PROFILE_TABLE).upsert(payload,{onConflict:'id'});btn.disabled=false;if(error){statusText(status,'تعذر حفظ الملف. تحقق من البيانات وحاول مجددًا.');return}const nextAvatar=avatarUrl(root.querySelector('#profilePageAvatar').value);const avatarBox=root.querySelector('.mirsad-profile-page__avatar');avatarBox.innerHTML=nextAvatar?`<img src="${safe(nextAvatar)}" alt="" loading="lazy">`:`<span>${safe(initial)}</span>`;avatarBox.querySelector('img')?.addEventListener('error',event=>{event.currentTarget.remove();const fallback=document.createElement('span');fallback.textContent=initial;avatarBox.appendChild(fallback)});const menuButton=document.querySelector('#mirsadUserMenu .mirsad-user-button');if(menuButton)menuButton.innerHTML=nextAvatar?`<img src="${safe(nextAvatar)}" alt="">`:`<span class="mirsad-user-initial">${safe(initial)}</span>`;statusText(status,'تم حفظ الملف بنجاح.')});
