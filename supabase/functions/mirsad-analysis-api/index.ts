...
const MIRASD_URL = Deno.env.get("MIRSAD_API_URL");
const MIRASD_KEY = Deno.env.get("MIRSAD_API_KEY");
...
Deno.serve(async (req: Request) => {
  ...
  if (!MIRASD_URL) {
    return json({ ok: false, error: "MIRSAD_API_URL is not configured" }, 503);
  }
  ...
  try {
    if (req.method === "POST") {
      ...
      if (!externalId || !headline || !content) {
        return json({
          ok: false,
          error: "external_id, headline and content are required"
        }, 400);
      }

      const response = await fetch(MIRASD_URL.replace(/\/$/, "") + "/articles", {
        method: "POST",
        headers: {
          "content-type": "application/json",
          ...(MIRASD_KEY ? { authorization: "Bearer " + MIRASD_KEY } : {})
        },
        body: JSON.stringify({
    ...
    const response = await fetch(
      MIRASD_URL.replace(/\/$/, "") + "/articles/" + encodeURIComponent(externalId) + "/analysis",
      {
        method: "GET",
        headers: {
