...
Deno.serve(async req=>{
  ...
  const prompt=`Translate each UI string into the standard written language identified by BCP-47 code ${lang}. Preserve placeholders, numbers, punctuation, and product names. Return JSON only as an array of objects with exactly source and translation fields, in the same order. Do not translate names such as Mirsad. STRINGS:\n${JSON.stringify(strings)}`;
