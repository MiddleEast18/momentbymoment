...
const publishedAt = (item: string, sourceKey = "") => { const maxFutureMs = 5 * 60 * 1000; const now = Date.now(); for (const tag of ["pubDate", "published", "updated", "dc:date"]) { const v = field(item, tag); if (!v) continue; const d = new Date(v); if (Number.isFinite(d.getTime())) { if (d.getTime() <= now + maxFutureMs) return d.toISOString(); if (sourceKey === "alaraby" && /(?:Z|[+\-]00:00)$/i.test(v)) { const localWallClock = v.replace(/(?:Z|[+\-]00:00)$/i, "+03:00"); const corrected = new Date(localWallClock); if (Number.isFinite(corrected.getTime()) && corrected.getTime() <= now + maxFutureMs) return corrected.toISOString(); } } const m = v.match(/(?:،\s*)?(\d{1,2})\s+(يناير|فبراير|مارس|أبريل|مايو|يونيو|يوليو|أغسطس|سبتمبر|أكتوبر|نوفمبر|ديسمبر)\s+(\d{4})\s+(\d{1,2}):(\d{2})\s+(ص|م)/); if (m) { const months: Record<string, number> = { يناير: 1, فبراير: 2, مارس: 3, أبريل: 4, مايو: 5, يونيو: 6, يوليو: 7, أغسطس: 8, سبتمبر: 9, أكتوبر: 10, نوفمبر: 11, ديسمبر: 12 }; let hour = Number(m[4]) % 12; if (m[6] === "م") hour += 12; const parsed = new Date(`${m[3]}-${String(months[m[2]]).padStart(2, "0")}-${m[1].padStart(2, "0")}T${String(hour).padStart(2, "0")}:${m[5]}:00+03:00`); if (Number.isFinite(parsed.getTime()) && parsed.getTime() <= now + maxFutureMs) return parsed.toISOString(); } } return new Date(now).toISOString(); };
...
Deno.serve(async req => {
  ...
  await withTimeout(db.from("ingest_runs").update({ status: errors.length ? "partial" : "completed", finished_at: new Date().toISOString(), items_seen: seen, items_written: written, notes: JSON.stringify({ duplicates, clusterUpdates, cleanupDeleted: 0, errors: errors.slice(0, 10) }) }).eq("id", runId), 15000, "finish_ingest_run");
    return reply({ ok: true, seen, written, duplicates, clusterUpdates, cleanupDeleted: 0, errors: errors.slice(0, 10) });
  } catch (error) {
