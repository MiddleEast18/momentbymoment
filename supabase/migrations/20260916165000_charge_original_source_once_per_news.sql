...
create unique index if not exists source_open_charges_news_once_idx
  on public.source_open_charges (user_id, source_kind, source_id)
...
revoke all on function public.open_original_source(text, uuid, text, uuid) from public, anon;
grant execute on function public.open_original_source(text, uuid, text, uuid) to authenticated;
