...
begin
  ...
  if prior.email_hash is not null then
    ...
    insert into public.user_unlocks(user_id, unlock_balance, unlimited_unlocks)
    values (new.id, 1000, false) on conflict (user_id) do nothing;
    insert into public.unlock_ledger(user_id, amount, event_type, reference_id)
...
as $function$
select exists (select 1 from auth.users u join public.profiles p on p.id=u.id where u.id=auth.uid() and p.onboarding_completed=false and u.created_at>=now()-interval '24 hours' and not exists (select 1 from public.first_signup_rewards r where r.user_id=u.id and r.reward_amount>0));
$function$;
