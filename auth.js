(() => {
  ...
  function injectStyles(){
    ...
    const s=document.createElement('style');s.id='mirsadAuthStyles';s.textContent=`
      ...
      .mirsad-otp-row{width:min(328px,calc(100% - 8px));max-width:328px;margin:0 auto;display:grid;grid-template-columns:repeat(6,minmax(0,1fr));gap:clamp(5px,2.5vw,8px);box-sizing:border-box;overflow:visible}.mirsad-otp-input{width:100%;min-width:0;box-sizing:border-box;height:54px;border-radius:12px;border:1px solid var(--panel-border-strong);background:rgba(255,255,255,.03);color:var(--text);text-align:center;font:600 22px var(--font-mono);outline:none}.mirsad-otp-input:focus{border-color:var(--gold)}@media (max-width:380px){.mirsad-otp-row{max-width:100%;gap:5px}.mirsad-otp-input{height:52px}}
  ...
  function showOtp(email){
    ...
    inputs.forEach((input,i)=>{input.addEventListener('input',()=>{input.value=input.value.replace(/\D/g,'').slice(-1);if(input.value&&inputs[i+1])inputs[i+1].focus()});input.addEventListener('keydown',e=>{if(e.key==='Backspace'&&!input.value&&inputs[i-1])inputs[i-1].focus()});input.addEventListener('paste',e=>{const v=(e.clipboardData?.getData('text')||'').replace(/\D/g,'').slice(0,6-i);if(!v)return;e.preventDefault();[...v].forEach((n,j)=>{if(inputs[i+j])inputs[i+j].value=n});inputs[Math.min(i+v.length,6)-1]?.focus()})});
